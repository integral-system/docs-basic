...
function CDTDocumentBasic::Make(level, page_w, page_h)
{
	...
	/* formula example from https://tex.stackexchange.com/questions/120271/alternatives-to-latex#120279 */
	...
	local nabla = {Text = Nabla, FontUniqueID = "F0011_STIX2M_NS0"};
	local deq = {Text = DeltaEq, FontUniqueID = "F0011_STIX2M_NS0"};
	local pd = {Text = PartialDifferentialItalic, FontUniqueID = "F0011_STIX2M_NS0"};
	...
	+ Gap()
	+ Tab + Above(Run(c) + Brack("s"), nabla) + Eq + Tab + Frac(pd, Run(pd) + s) + ThinSp + Brack(Sub("α", "d") + Frac(Run(pd) + ThinSp + f + c, Run(pd) + s)) + Sum + Sub("α", "R") + ThinSp + Frac(c, tr()) + Mul + Frac(Sup(pd, 2) + ThinSp + f + ThinSp + tr(), Run(pd) + Sup("s", 2)) + Sum + Frac(Sub("α", "R"), 2) + Mul + Frac(Run(pd) + ThinSp + f + ThinSp + tr(), Run(pd) + s) + Mul + Frac(pd, Run(pd) + s) + ThinSp + Brack(Frac(c, tr())) + Diff + Brack(Frac(1, Sub("τ", Run("CR"))) + Sum + Sub(f, Run("CCR"))) + Mul + Frac(3, tr()) + Mul + f_c_delta() + ","
	...
	math + Gap()
	+ "avec" + Tab + f + Arg("(s)") + deq + Frac("b" + Diff + 3, Arg("b") + Diff + tr("s")) + ThinSp + ";"

	+ Gap()
	+ Tab + Sub("α", "R") + Brack("s") + deq + Frac(4, Sub("K", "R") + ThinSp + Sup(Pi, 2) + Sup("M", 2)) + ThinSp + ";"

	+ Gap()
	+ Tab + Sub("α", "d") + Brack("s") + deq
	...
	+ Gap()
	+ Tab + Sub(f, Run("CCR")) + deq + MinusSp + Frac(Integral(EvalBarB(Frac("D", "Dt") + ThinSp + a(), Run("stretch"), {SymMask = 1}) + ThinSp + "d" + s, Minus + 1, Plus + 1) , Integral(a() + ThinSp + "d" + s, Minus + 1, Plus + 1)) + ThinSp + ";"

	+ Gap()
	+ Tab + a() + deq + g() + ThinSp + Brack(Run(MinusSp) + "(b" + Diff + "3) log" + ThinSp + Brack(Run(1) + Diff + Frac(tr(), "b")) + Diff + "log" + Brack(Run("det") + Brack(c))) + ThinSp + ".";
...
local doc = CDTDocumentBasic("../../common/fontmaps/stix-2.inf");
